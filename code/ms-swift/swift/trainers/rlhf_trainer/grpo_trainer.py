...
class GRPOTrainer(RLHFTrainerMixin, SwiftMixin, HFGRPOTrainer):
    ...
    def __init__(self,
        ...
        self.semantic_threshold_static = None
        self.semantic_threshold_dynamic = None
        self.semantic_classifier = MedImageInsight(
    ...
    def _dynamic_sampling_o(self, inputs, rewards, rewards_per_func, completions):
    ...
    def _dynamic_sampling(self, inputs, rewards, rewards_per_func, completions):
        ...
        while resample_count < self.args.max_resample_times:
            ...
            mpo = False
            if len(inputs[0]['messages']) > 2 or len(inputs[1]['messages']) > 2:
                mpo = True
                # 将奖励按对话轮次分组（假设每样本生成两轮对话）
                grouped_rewards = rewards.view(-1, self.num_generations)

                # 计算跨轮奖励协同指标
                round1_rewards = rewards_per_func[::2, 0]  # 奇数索引是首轮奖励首轮奖励
                round2_rewards = rewards_per_func[1::2, 0]  # 次轮奖励
                reward_synergy = round1_rewards * round2_rewards  # 协同效应

                # 动态阈值（取当前批次协同值的中位数）
                synergy_threshold = reward_synergy.quantile(0.5).item()
                # print(f"Synergy threshold: {synergy_threshold}")

                # 有效性条件：组内方差>0且协同值超过阈值 若小于0.5取0.5 若大于0.8取0.8
                synergy_threshold = max(0.5, min(0.8, synergy_threshold))
                valid_groups = (grouped_rewards.std(dim=1) > 0) & (grouped_rewards.mean(dim=1) > synergy_threshold)
                valid_mask = valid_groups.repeat_interleave(self.num_generations)
            ...
            # if mpo:
            #     with torch.no_grad():
            #         anchor_mask = round1_rewards > rewards_per_func.median()
            #         inputs = [inp for inp, m in zip(inputs, anchor_mask) if m]
            #     new_inputs = next(self.resample_iterator)
            #     new_inputs = Trainer._prepare_inputs(self, new_inputs)
            #     inputs += new_inputs[:self.global_train_batch_size - len(inputs)]
            # else:
            inputs = next(self.resample_iterator)
            inputs = Trainer._prepare_inputs(self, inputs)
        ...
        if len(valid_samples) >= self.global_train_batch_size:
            ...
            # logger.warning(f'There are still std=0 groups present after {self.args.max_resample_times} retries.')
            logger.warning(f'There are still std=0 groups or low reward_synergy present after {self.args.max_resample_times} retries.')
            inputs, rewards, rewards_per_func, completions = origin_data

        return inputs, rewards, rewards_per_func, completions
    
    def _dynamic_sampling_2(self, inputs, rewards, rewards_per_func, completions):
        ...
        reflection_enabled = self.args.reflection_enabled
        medimageinsight_model = None
        if reflection_enabled:
            medimageinsight_model = self.semantic_classifier
            logger.info("Reflection mechanism enabled with MedImageInsight")

        static_threshold = None
        dynamic_threshold = None

        if reflection_enabled and medimageinsight_model is not None:
            if self.state.global_step < getattr(self.args, 'reflection_static_steps', 1000):
                static_threshold = self._compute_static_threshold(inputs)
            else:
                dynamic_threshold = self._compute_dynamic_threshold(inputs, medimageinsight_model)

        while resample_count < getattr(self.args, 'max_resample_times', 5):
            grouped_rewards = rewards.view(-1, self.num_generations)
            group_std = grouped_rewards.std(dim=1)

            semantic_mask = torch.ones_like(group_std, dtype=torch.bool)
            if reflection_enabled and medimageinsight_model is not None:
                semantic_mask = self._check_semantic_consistency(
                    inputs, completions, medimageinsight_model,
                    static_threshold, dynamic_threshold
                )

            valid_group_mask = (group_std > 0) & semantic_mask
            valid_mask = valid_group_mask.repeat_interleave(self.num_generations)

            all_inputs = gather_object(inputs)
            for i, (inp, mask) in enumerate(zip(all_inputs, valid_mask)):
                if mask:
                    valid_samples.append(inp)
                    valid_completions.append(inp['messages'][-1]['content'])
            ...
            if len(valid_samples) >= self.effective_train_batch_size:
                logger.info(
                    f"DAPO+Reflection: Found {len(valid_samples)} valid samples after {resample_count} resamples")
                break

            logger.info(f"DAPO+Reflection: Resampling iteration {resample_count + 1}")

            try:
                new_inputs = next(self.resample_iterator)
            except StopIteration:
                self.resample_iterator = iter(self.get_resample_dataloader())
                new_inputs = next(self.resample_iterator)

            new_inputs = Trainer._prepare_inputs(self, new_inputs)
            new_inputs = self._generate_completions(new_inputs)
            new_rewards_per_func, new_rewards, new_completions = self._score_completions(new_inputs)
            inputs = new_inputs
            rewards = new_rewards
            ...
            if reflection_enabled and medimageinsight_model is not None and self.state.global_step >= getattr(self.args,
                                                                                                              'reflection_static_steps',
                                                                                                              1000):
                dynamic_threshold = self._compute_dynamic_threshold(inputs, medimageinsight_model)
        ...
        if len(valid_samples) >= self.effective_train_batch_size:
            ...
            valid_ratio = len(valid_samples) / self.effective_train_batch_size
            logger.info(f"DAPO+Reflection: Valid sample ratio: {valid_ratio:.2%}, Resample count: {resample_count}")
        else:
            logger.warning(f'DAPO+Reflection: Still have std=0 or low-semantic groups after {resample_count} retries.')
            inputs, rewards, rewards_per_func, completions = origin_data

        return inputs, rewards, rewards_per_func, completions

    def _compute_static_threshold(self, inputs):
        return 0.75  # example

    def _compute_dynamic_threshold(self, inputs, medimageinsight_model):
        try:
            ...
            for inp in inputs:
                if 'messages' in inp:
                    ...
                    for msg in inp['messages']:
                        if msg.get('content'):
                            conversation_text += f"{msg.get('role', 'user')}: {msg['content']}\n"
                    if conversation_text:
                        all_texts.append(conversation_text.strip())

            embeddings = medimageinsight_model.encode(texts=all_texts)
            ...
            if embeddings is not None and len(embeddings) > 0:
                ...
                if similarities:
                    threshold = np.quantile(similarities, 0.75)
                    threshold = max(0.6, min(0.9, threshold))
                    logger.info(f"threshold update: {threshold:.4f}")
                    return threshold
    ...
    def _check_semantic_consistency(self, inputs, completions, medimageinsight_model, static_threshold,
                                    dynamic_threshold):
        try:
            if self.state.global_step < getattr(self.args, 'reflection_static_steps', 1000):
                threshold = static_threshold or 0.75
                threshold_type = "static"
            else:
                threshold = dynamic_threshold or 0.75
                threshold_type = "dynamic"

            batch_size = len(inputs) // self.num_generations
            semantic_mask = torch.ones(batch_size, dtype=torch.bool)

            for group_idx in range(batch_size):
                group_start = group_idx * self.num_generations
                group_end = group_start + self.num_generations
                group_completions = completions[group_start:group_end]

                if len(group_completions) >= 2:
                    try:
                        embeddings = medimageinsight_model.encode_text(group_completions)
                        ...
                        if embeddings is not None and len(embeddings) >= 2:
                            ...
                            if similarities:
                                group_similarity = np.mean(similarities)
                                semantic_mask[group_idx] = group_similarity >= threshold
                            else:
                                semantic_mask[group_idx] = True
                        else:
                            semantic_mask[group_idx] = True
                    ...
                    semantic_mask[group_idx] = True
            consistent_ratio = semantic_mask.float().mean().item()
            logger.info(f"semantic check {threshold_type}={threshold:.4f}: "
                        f"{consistent_ratio:.2%} pass")

            return semantic_mask

        except Exception as e:
            logger.error(f"failed: {e}")
            batch_size = len(inputs) // self.num_generations if len(inputs) % self.num_generations == 0 else len(inputs)
